...
router.post('/issuer/create_schema', auth.isLoggedIn, async function (req, res) {
    ...
    if (attributes && photo_url) {
        var add_url = '    "photo_url:' + photo_url + '",\n';
        var position = 2;
        attributes = [attributes.slice(0, position), add_url, attributes.slice(position)].join('');
    }
    await indy.issuer.createSchema(req.body.name_of_schema, req.body.version, attributes);
